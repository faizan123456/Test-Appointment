...
const tokenSecret = process?.env?.SECRET ? process?.env?.JWT_SECRET : "jwt_test_secret_verse";
...
module.exports = {
    authenticateToken: async (req, res, next) => {
        ...
        jwt.verify(token, tokenSecret, async (err, result) => {
          ...
          //  let userFilter = result.role === 'user' ? {_id: result.id} : { walletAddress: result.walletAddress };
            let userData;
            if(result.role === 'guest' ){
                userData = {
                 ...
                 userData = await User.findOne({_id: result.id});
